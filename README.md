...
This is a superset of LOLCODE that was conjured into existence by a human saying "make LOLCODE less painful" to an AI, which then proceeded to write thousands of lines of JavaScript transpiler code at 3am. We're not pretending otherwise.
...
📘 **[Language Specification](docs/LULCODE-SPECIFICATION-ULTRA-FINAL.md)** - 1,200 lines of formal grammar in EBNF notation, complete transpilation rules, and examples. Implementation is 100% compliant with spec.

📊 **[Development Status](STATUS.md)** - Full development history, milestones, and current progress.

⚙️ **[Installation Guide](INSTALLATION.md)** - Simple installation instructions (just npm install!).
...
```
LULCODE (.lul) ──[ LULCODE transpiler ]──► LOLCODE (.lol) ──[ lol-coffee interpreter ]──► output
```

Two steps, one of them a transpile:

1. **LULCODE → LOLCODE.** The LULCODE transpiler is JavaScript code in `src/transform.js`. It reads your `.lul` file and emits a `.lol` file that's valid LOLCODE 1.2.
2. **LOLCODE → output.** The bundled lol-coffee runtime *interprets* the LOLCODE. It tokenizes, parses to an AST, compiles to its own VM instructions, and runs them. Your LOLCODE never gets turned into CoffeeScript or JavaScript along the way.

The "coffee" in lol-coffee just refers to the language its authors wrote it in. They compiled their CoffeeScript to JavaScript once before publishing; we ship the resulting JS files (`runtime/lol-coffee/*.js`) and load them at runtime. No CoffeeScript toolchain runs when you execute a LULCODE program.
...
### 🚧 First-Class Functions (Future Work)
...
Yeah, surprisingly. The AI wrote 48 tests and they all pass. Here's what exists:
...
- ✅ **Formal specification** - 1,200 lines of EBNF grammar and transpilation rules

Recent additions (all generated by having conversations with Claude):
- **May 20, 2026**: Formal language specification complete, 100% implementation compliance
- **May 20, 2026**: Arrays work, full test suite passes, CLI is fancy with colors and everything
...
Boom, you have the `lulcode` command. Everything you need is bundled - no external dependencies!
...
Now you can modify the transpiler and make it better or worse. The LOLCODE runtime is bundled in `runtime/lol-coffee/`.
...
WTV aliceScore ITZ players["Alice"]
ORLY aliceScore >= 100
  VISIBLE "Alice wins with {aliceScore} points!"
NOWAI
  VISIBLE "Alice has {aliceScore} points"
KTHX
...
Heads up: `{...}` interpolation only works with a plain identifier inside the braces — pull BUKKIT properties into a variable first like `aliceScore` above. Putting `{players["Alice"]}` directly in a string won't expand.
...
I HAS A aliceScore ITZ players'Z Alice
BOTH SAEM aliceScore AN BIGGR OF aliceScore AN 100, O RLY?
  YA RLY
  VISIBLE "Alice wins with :{aliceScore} points!"
  NO WAI
  VISIBLE "Alice has :{aliceScore} points"
...
48 tests. All pass. All validated against the formal spec. I'm as surprised as you are.
...
- [lol-coffee](https://github.com/replit-archive/lol-coffee) - The LOLCODE interpreter bundled with LULCODE
- [BUKKIT Discussion from 2018](https://github.com/justinmeza/lolcode-spec/issues/11) - Where people asked for arrays and got nothing for years
